...
def get_house_percommunity(city, communityname):
    ...
    url = baseUrl + u"ershoufang/rs" + \
          request.quote(communityname.encode('utf8')) + "/"
    ...
    for page in range(total_pages):
        ...
        for name in nameList:  # per house loop
            ...
            # models.Hisprice.insert(houseID=info_dict['houseID'], totalPrice=info_dict['totalPrice']).upsert().execute()
    ...
    for page in range(total_pages):
        ...
        for ultag in soup.findAll("ul", {"class": "listContent"}):
            for name in ultag.find_all('li'):
                ...
                data_source.append(info_dict)
                # models.Sellinfo.insert(**info_dict).upsert().execute()
...
def get_community_perregion(city, regionname=u'xicheng'):
    ...
    for page in range(total_pages):
        ...
        for name in nameList:  # Per house loop
            ...
            data_source.append(info_dict)
            # models.Community.insert(**info_dict).upsert().execute()
        with models.database.atomic():
            if data_source:
    ...
    for page in range(total_pages):
        ...
        for ultag in soup.findAll("ul", {"class": "house-lst"}):
            for name in ultag.find_all('li'):
                ...
                data_source.append(info_dict)
                # models.Rentinfo.insert(**info_dict).upsert().execute()
